...
- 用户可按供应商收款资料线下付款并声明，由供应商确认；当账单冻结了 Binance UID、商家绑定了严格只读的 Binance API Key 且 Router 总开关启用时，也可创建 15 分钟 USDT 付款 intent，以四位小数唯一尾差自动识别入账。自动与人工确认共用同一原子结算核心，任一方胜出后都会阻止另一方重复入账；逾期会限制用户继续使用市场赊账。争议由 Router 管理员裁决,账单也可由管理员作废。
...
- 租用后,Router 才通过 pending Share edit 在 Server 上创建 `routerShareMarket` 管理的 `shareto` entitlement。普通 Share 编辑不能修改或删除仍为 active 的这类 entitlement；租约结束后的 tombstone 不再占用该邮箱，可供普通 Manual `shareto` 复用。
...
| `notification_task` | 5s | 离线/恢复通知 outbox（邮件 / Telegram / Bark） |
...
2. **aggregate** —— 同收件人事件在窗口内（默认 60s）合并为一个批次；风暴检测触发时进一步合并为 digest，并解析账号当前唯一选中的可用渠道，不可用时回落 Email
3. **deliver** —— worker 以 90 秒租约认领批次，按批次自身的 `channel` 经 Resend、Telegram Bot API 或 Bark `/push` 发送
...
- **通用 outbox**:`notification_deliveries` 保存具体渠道、脱敏归属、冻结 payload 与结构化失败信息；`notification_delivery_items` 以 event/recipient/channel 唯一。正常聚合只生成当前单选渠道的一份投递；渠道切换或目标失效会取消尚未开始的旧投递，再把原事件释放并重新聚合到新渠道。
...
- **回落而非丢弃**:目标为空、Telegram Bot 不可用或 Bark Provider 不可用时至少生成 Email。Registration lane 固定走邮件，首次登录口令提示不进入第三方 Push 渠道。
- **并发关闭语义**:禁用渠道或解绑会取消 pending/retry 及未开始的 reservation；已经进入 provider 调用的 started attempt 可完成,避免在未知发送结果下重复投递。
- **不可达处理**:Telegram 明确返回 chat 不可达，或 Bark 明确返回已知的 Device Key 查询失败时，事务会校验 delivery 归属、Provider identity、目标与 revision，结束当前 attempt，只使匹配绑定失效，并释放原事件生成一次邮件回落。普通 Bark 400/404 或 401/403 归为 Provider 配置故障，不会删除用户绑定。

Telegram 绑定链路：`POST /v1/me/notifications/telegram/bind-link` 只持久化 128 位 token 的 SHA-256，返回基于已验证 Bot username 的深链。已使用或撤销 token 保留 7 天，所以反复申请/撤销不能绕过用户与 IP 小时限额。polling 与 webhook 都先以 `(bot_id, update_id)` 幂等写入 `telegram_inbound_updates`；polling cursor 与整批 insert 在同一事务推进，handler 使用租约重试并且不会因单条失败丢弃后续输入。Bot token 热更新后先调用 `getMe`：Bot ID 不变时保留绑定并恢复对应阻断投递，Bot ID 变化时撤销旧链接、使旧绑定失效并启用邮件回落。该 Bot 与下节的运维告警 Telegram adapter 配置和状态相互独立。

Bark 绑定链路：`POST /v1/me/notifications/bark` 只接受管理员指定 Server 下的完整 Push URL，持久化脱敏的用户/IP 尝试记录并先发送验证消息。成功后 Device Key 以 XChaCha20-Poly1305 加密，AAD 绑定用户、credential revision 与 Provider identity；完整 Push URL 不回显。主密钥和版本是启动期边界，修改后绑定、选择和自动投递都会 fail closed，直到 Router 重启；Provider generation 还会阻止验证期间配置离开再恢复造成 ABA。Server 或密钥身份变化会使旧绑定失效，用户需重新绑定。连续系统性失败会打开持久熔断，显式绑定或管理员测试可探测并提前恢复。绑定尝试与 Telegram token 审计均保留 7 天。
