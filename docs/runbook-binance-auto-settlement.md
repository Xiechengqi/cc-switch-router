...
紧急停止只需把总开关改成 `disabled` 并重启。启动事务会立即取消已有活动 intent，并把账户级模式全部持久化降为 `shadow`；重新启用总开关后仍须逐商家重新绑定才能恢复自动结算。同 UID 严格复验重绑会保留 `global_settlement_disabled` intent 的迟到监控；如果买家在关闭边界仍完成了转账，重新启用轮询后只会进入人工对账而不会自动改账。买家始终可以选择人工付款。单个商家可在账户页停用；删除操作还会清除加密凭据。
