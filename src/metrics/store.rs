...
fn insert_llm_request_metric(conn: &Connection, metric: &LlmRequestMetric) -> Result<(), AppError> {
    conn.execute(
        ...
        ON CONFLICT(request_id) DO UPDATE SET
            ...
            market_email = COALESCE(excluded.market_email, llm_request_metrics.market_email),
            share_id = COALESCE(excluded.share_id, llm_request_metrics.share_id),
            subdomain = COALESCE(excluded.subdomain, llm_request_metrics.subdomain),
            app_type = COALESCE(excluded.app_type, llm_request_metrics.app_type),
            provider = COALESCE(excluded.provider, llm_request_metrics.provider),
            ...
            stream_started = MAX(stream_started, excluded.stream_started),
            stream_completed = MAX(stream_completed, excluded.stream_completed),
            input_tokens = COALESCE(excluded.input_tokens, llm_request_metrics.input_tokens),
            output_tokens = COALESCE(excluded.output_tokens, llm_request_metrics.output_tokens),
            total_tokens = COALESCE(excluded.total_tokens, llm_request_metrics.total_tokens),
            cache_read_tokens = COALESCE(excluded.cache_read_tokens, llm_request_metrics.cache_read_tokens),
            cache_write_tokens = COALESCE(excluded.cache_write_tokens, llm_request_metrics.cache_write_tokens),
            reasoning_tokens = COALESCE(excluded.reasoning_tokens, llm_request_metrics.reasoning_tokens),
            estimated_cost_usd = COALESCE(excluded.estimated_cost_usd, llm_request_metrics.estimated_cost_usd)",
        params![
            metric.timestamp,
            metric.request_id,
            metric.route_type,
