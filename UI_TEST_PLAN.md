...
| SM-12a | 已登录租客有进行中租用 | 打开「我的租用」 | 「进行中的租用」按 Share 卡片网格展示，一 Share 一卡；只显示自己有权限的 Share，不含历史租用 |
| SM-12b | listing 已停止但仍有该租客的进行中租约 | 打开「我的租用」 | 该 Share 仍出现在进行中卡片中，即使选购拼车位已不再展示 |
| SM-12c | owner 有挂售中和已停止 listing | 打开「我的挂售」 | 两区都是 Share 卡片；卡片上可群聊、添加拼车位、停止/恢复/删除挂售；座位级操作在点开卡片后处理 |
...
| SM-15 | 租约非终态 | 我的租用卡片 / 挂售卡片抽屉 | 有 subdomain 时显示「打开 Share」并可跳转 |
| SM-16 | 已登录买家不在 owner 白名单,Share 在线且座位可用 | 点「租用」 | 保留租用按钮；弹出中性授权引导而非红色英文错误，明确只有白名单用户可租用并显示当前登录邮箱；点主操作打开该 Share 对应的 Client 聊天室 |
| SM-17 | 新建/编辑免费或付费拼车位 | 分别输入 1、365、0、366 天并切换无固定期限，再切换免费/付费 | 免费和付费均允许 1/365；0/366 前后端拒绝；无固定期限请求省略 `serviceDurationDays`；用户手动修改期限后切换价格模式不会覆盖该选择 |
| SM-18 | 新建/编辑拼车位 | Token 限额留空后再设置数值 | 留空时周期控件隐藏且请求归一为 `lifetime`；设置数值后周期控件出现，只展示 Server 支持的周期 |
| SM-19 | 固定与无固定期限租约并存 | 查看 All / Mine / My rentals / Account Share | 免费和付费报价都显示冻结服务期限；固定期限显示从租用创建时间起算的开始、结束和剩余时间，无固定期限明确标注 |
| SM-20 | 在线且已获准入的可用座位 | 点租用 | 先出现确认框，明确 Owner、Share/座位、在线状态、USD 日费或免费报价、独立服务期限及其从确认时起算；未确认前不提交租用请求 |
...
| SM-E2E-07 | 租用 1 天固定期限座位，Server 延迟应用 grant | 到期时间从点击确认并成功创建租约时开始，edit-ack 不顺延；grant policy 与 Subscription 使用同一绝对到期时间；临期事件只产生一次 |
| SM-E2E-08 | 免费或付费服务期限到期 | `grant_pending`、活跃、账单暂停或恢复中租约均不会再恢复访问；必要时进入 revoke，Server ack 后座位恢复可租；付费合约终止；回收失败时保持失败/回收态 |
| SM-E2E-09 | 同一 renter 租用该 owner 的多个 Share/Client Host 并达到阈值 | 生成一张含多个服务明细的聚合账单并暂停相关服务；完整收款资料只在该账单中出现 |
| SM-E2E-10 | 付费固定期限到期前保持健康，分别让账务 worker 或 Share worker 先处理 | 两种顺序都只累计到绝对 `expiresAt`；最后一个账务周期不遗漏，也不累计到期后的秒数 |
...
| H-55 | Quote 同时含免费和付费 Host | 进入最终确认阶段 | 每台显示 Owner 与冻结报价；统一说明免费期限从激活开始、付费 Host 独立享受 12 小时健康试用，并按各供应商账户分别聚合 Share/Host 费用 |
...
| AS-09 | 固定期限与无固定期限租约并存 | 查看 User / Provider 卡片 | 免费和付费报价都显示服务期限；固定期限显示租用创建和到期时间，无固定期限明确标注 |
...
| MA-04 | 供应商给买家有限额度 | 保存后再更新额度 | 只显示并保存 USD 额度,revision 递增；后续账户使用新额度协调状态 |
...
| MA-15 | 同时修改多名买家的状态、准入或 USD 授信 | 观察全局按钮，先重置再重新修改并保存 | 无语义变更时「保存」和「重置」禁用；重置恢复服务端快照；保存一次提交全部脏行并刷新 revision；无限额度仍需逐项确认风险 |
| MA-16 | 白名单买家尚无权限 | 从 Share/Host 引导点「申请加入白名单」 | 创建一条「申请准入」记录；重复点击返回同一待处理申请，供应商导航和市场准备显示数量角标 |
| MA-17 | 付费申请且买家无额度 | 供应商点击准入并分别选择有限/无限额度 | 弹窗同时配置对应付费作用域和 USD 额度；成功后一次刷新为已准入，买家可租；无限额度必须确认风险 |
| MA-18 | 付费申请 | 只提交批准、不提交有效额度，或额度写入发生冲突 | 整个事务失败；申请仍待处理，不新增关系/规则，也不出现只准入未授信状态 |
| MA-19 | 待处理申请 | 供应商拒绝但原因为空、填写原因后拒绝、买家取消 | 空原因无法提交；拒绝原因持久化并触发 24 小时冷却；买家可按当前 revision 取消自己的申请 |
| MA-20 | 两名买家均有未保存修改，第二名 revision 已过期 | 点全局保存 | 原子批量请求整体冲突，第一名也不落库；草稿保留并提示重新加载，不出现部分保存 |

### 9.3 统一市场账务(MB)

覆盖 `account-billing-page.tsx`。Share 与 Client Host 按「买方 + 供应商」共用唯一 USD 赊账账户；商品页不承担付款交互。

| ID | 前置 | 步骤 | 预期 |
|---|---|---|---|
| MB-01 | 未登录 | 打开 `/account/billing` | 提示登录,不显示任何账户或收款快照 |
| MB-02 | 供应商 | 配置 USD 付款宽限时间 | 1-720 整数小时可保存；非法值被前端阻止 |
| MB-03 | 仅有收款资料或仅有 USD 付款宽限 | 发布付费 Share/Host | 均被阻止；两项都配置后才可发布 |
| MB-04 | 同供应商有多个付费 Share/Host | 查看应付账户 | 只出现一个 USD 账户，列出多个服务、每日费率、健康时长试用与累计余额 |
| MB-05 | 尚未出账 | 查看账户及各商品页 | 不显示完整账号、地址或二维码；只显示付款方式种类和联系方式 |
| MB-06 | Settings 汇率为 7.5 且有未出账余额 | 买方点「主动清账」并确认 | 生成一张聚合账单，关联服务暂停；总额和明细同时显示 USD/CNY，明细人民币金额之和等于按 7.5 四舍五入后的总额 |
...
| MB-20 | Settings 使用默认汇率 7 | 打开「收款信息」和「市场账务」 | 均显示“美元人民币汇率采用 1:7，推荐使用美元渠道付款” |
| MB-21 | 已有按 7.5 生成的账单 | 管理员在 Settings 将汇率改为 8，再刷新账务 | 未出账余额和页面说明使用 1:8；旧账单仍显示冻结的 1:7.5 及原人民币总额和明细，新账单使用 1:8 |
| MB-22 | 同时有应付、待确认和历史账单 | 切换五类账务视图 | 「待我处理」只集中可执行事项；应付/应收分离；历史按账户加载；管理员争议仅管理员可见 |
| MB-23 | 存在逾期限制 | 点击限制横幅中的账单按钮 | 自动切到「待我处理」并滚动定位对应账户/账单 |
| MB-24 | 依次打开付款、确认、拒绝、争议、永久关闭弹窗 | 观察主按钮并尝试空原因提交 | 主按钮使用具体动作文字；拒绝、争议、作废和关闭使用危险色；必填原因为空时不可提交，不再统一显示「验证」 |

### 9.4 市场准备(MR)

覆盖 `account-market-readiness-page.tsx`。该页只聚合已有资源，不维护独立状态。

| ID | 前置 | 步骤 | 预期 |
|---|---|---|---|
| MR-01 | 已登录供应商 | 打开 `/account/market-readiness` | 并行汇总收款方式、待准入、账务动作、逾期数和四项准入策略；刷新按钮可重新读取 |
| MR-02 | 无收款方式 / 有收款方式 | 观察收款行 | 分别显示需要配置 / 已就绪，并可深链到收款信息 |
| MR-03 | 有待准入和待确认付款 | 观察导航与聚合行 | 市场准备和市场准入均显示待准入角标；聚合行显示账务数量并深链到正确页面 |
| MR-04 | 四作用域采用混合模式 | 查看策略表 | Share/Client Host 与免费/付费四行完整展示，仅使用「仅可信买家 / 默认开放」术语和各自 revision |
